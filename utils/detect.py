...
def wall_filter(gray, kernel=3, opening_iter=3, dilate_iter=3):
    """
    Filter walls
...
def find_details(img, noise_removal_threshold=50, corners_threshold=0.01,
    ...
    for label in unique:
        component = labels == label
        if img[component].sum() == 0 or np.count_nonzero(component) < gap_in_wall_min_threshold or np.count_nonzero(
                component) > gap_in_wall_max_threshold:
            color = 0
        else:
